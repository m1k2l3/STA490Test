...
data_raw <- read_sas("tr_quan_national_deid.sas7bdat",NULL)
```
...
data_raw_target <- data_raw_target %>%
  mutate(
    q15 = case_when(
      q15 == 1 ~ "Metastatic",
      q15 == 2 ~ "Not metastatic but spread afterwards",
      q15 == 3 ~ "Not metastatic and not spread",
      q15 == 4 ~ "Unsure",
...
data_raw_target$q24c <- factor(data_raw_target$q24c, levels = c("No response/Other", "Very poor","Poor", "Fair", "Good", "Very good"), ordered = TRUE)
...
data_raw_target$q31f <- factor(data_raw_target$q31f, levels = c("No response/Other", "Strongly disagree","Somewhat disagree", "Neither agree nor disagree", "Somewhat agree", "Strongly agree"), ordered = TRUE)
...
data_raw_target <- data_raw_target %>%
  mutate(
    q38 = case_when(
      q38 %in% c(3, 4, 5, 6, 23, 37, 42, 58) ~ "Access/Knowledge barriers",           
      q38 %in% c(9, 10, 11, 12, 21, 35, 36, 53) ~ "Thinks no help",             
      q38 %in% c(13, 14, 15, 16, 17, 57) ~ "Not a concern", 
      q38 %in% c(1, 2, 7, 8, 18, 19, 24, 25, 26, 27, 28, 29, 30, 55) ~ "Belief or personal preferences", 
      q38 %in% c(31, 32, 44, 46, 47, 48, 49, 50, 51) ~ "Had support from others", 
      q38 %in% c(22, 33, 34, 38, 39, 40, 41, 43, 45, 52, 54, 56) ~ "Self-related/Unspecified", 
      TRUE ~ "No response/Other"
    )
  )
...
data_raw_target <- data_raw_target %>%
  mutate(
    q48 = case_when(
      q48 %in% c(3, 4, 5, 6, 9, 10, 32, 37, 38, 40, 41, 42, 51, 56) ~ "Access/Knowledge barriers",
      q48 %in% c(13, 26, 27, 52) ~ "Thinks no help",
      q48 %in% c(23, 57, 58, 59, 60, 55) ~ "Not a concern",
      q48 %in% c(1, 2, 7, 8, 11, 12, 24, 25, 30, 31, 34, 47, 49, 50, 53) ~ "Belief or personal preferences",
      q48 %in% c(15, 16, 17, 18, 19, 21, 22, 35, 44, 45, 46 ) ~ "Had support from others", 
      q48 %in% c(28, 29, 33, 36, 39, 43, 48, 54) ~ "Self-related/Unspecified", 
      TRUE ~ "No response/Other"
    )
  )
...
data_raw_target <- data_raw_target %>%
  mutate(
    q58 = case_when(
      q58 %in% c(3, 4, 5, 6, 9, 10, 24, 25, 27, 28, 29, 30, 31, 32, 44, 47) ~  "Access/Knowledge barriers",
      q58 %in% c(26, 27, 29, 34, 35) ~ "Thinks no help",
      q58 %in% c(19, 46, 48, 49, 50, 51) ~ "Not a concern",
      q58 %in% c(1, 2, 7, 8, 11, 12, 20, 21, 23, 36, 37, 42, 43) ~ "Belief or personal preferences",
      q58 %in% c(14, 15, 16, 17, 22, 39, 40, 41) ~ "Had support from others",
      q58 %in% c(33, 38, 45, 46, 47) ~  "Self-related/Unspecified",
      TRUE ~ "No response/Other"
    )
  )
...
data_raw_target <- data_raw_target %>%
  mutate(
    q80 = case_when(
      ...
      q80 == 2 ~ "25,000 to 50,000",
      q80 == 3 ~ "50,000 to 75,000",
      q80 == 4 ~ "75,000 to 125,000",
      q80 == 5 ~ "125,000 or more",
      q80 == 6 ~ "No response/Other",
